...
def detection(nb_epochs=60, batch_size=128,
    ...
    if (FLAGS.label_type == 'type1'):
        acc = model_eval(sess, x, y, model(x), 1. - x_test, y_test, batch_size=batch_size)
    else:
        preds = model.predict(1 - x_test, batch_size=128)
        res = (np.argmax(preds, axis=-1) - nb_classes // 2) == np.argmax(y_test, axis=-1)
    ...
    np.save("cache/%s_%s_%s.npy" % (FLAGS.dataset, FLAGS.attack_type, cache_name[FLAGS.detection_type]), x_adv_test)
    distortion = 0
    ...
    elif (FLAGS.detection_type == 'mlloo'):
        de.build_detection_dataset(model, x_train, y_train, x_test, y_test, x_adv_test, selected_idx)


    #de.evaluate_detections(data_name)
    #unknow attack
    de.evaluate_detections_fgsm(data_name)
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string("detection_type", "mlloo", "which detection type to use")
