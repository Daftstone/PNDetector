...
STDEVS = {
    'mnist': {'fgsm': 0.310, 'bim-a': 0.128, 'bim-b': 0.265, 'lbfgs': 0.250, 'cw': 0.228},
...
def get_deep_representations(model, X, batch_size=256):
    ...
    for i in range(n_batches):
        # output[i * batch_size:(i + 1) * batch_size] = \
        #     get_encoding([X[i * batch_size:(i + 1) * batch_size], 0])[0].reshape(batch_size, output_dim)
        temp = get_encoding([X[i * batch_size:(i + 1) * batch_size], 0])[0]
        output[i * batch_size:(i + 1) * batch_size] = temp.reshape(len(temp), output_dim)
    return output
